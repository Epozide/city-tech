...
<nav area-label="breadcrumb">
	...
	<div class="card">
		<div class="card-header">{{ $order->order_number }} <strong class="ml-5">Worth {{ $order->billing_total }} EGP</strong></div>
		<div class="card-body">
			...
			<form action="{{ route('orders.update', $order->id) }}" method="post">
				...
				<div class="form-group">
					...
					<select class="form-control" name="status">
						<option class="text-capitalize" value="{{ $order->status }}">Currently: {{ $order->status }}</option>
