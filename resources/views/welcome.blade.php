...
<title>{{ $systemName->name }}: Electronics Store, Computers, Laptops, Accessories, Bags, Cameras, Networking, and Service Provider.</title>
<meta charset="UTF-8">
        ...
        <div class="container">
            ...
            <div class="product-slider owl-carousel">
                ...
                <div class="product-item">
                    ...
                    <div class="pi-text">
						<a href='{{ route('single-product', $p->slug) }}' style="color:black;">{{$p->name}}</a>
						<a href='{{ route('single-product', $p->slug) }}' style="color:#9999ff;">{{$p->price}} EGP</a>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <div class="col-lg-3 col-md-4 col-sm-6 col-xs-6">
                    <div class="product-item">
                        <div class="pi-pic">
                            ...
                            <div class="pi-links">
                                <form action="{{ route('cart.store') }}" method="post">
                                    ...
                                    <input type="hidden" name="price" value="{{$p->price}}">
                                    <input type="hidden" name="quantity" value="1">
                                    <button type="submit" class="add-card"><i class="flaticon-bag"></i><span>اضافة للعربة</span></button>
                                </form>
                                <form action="{{ route('wishlist.store') }}" method="post">
                                    @csrf
                                    <input type="hidden" name="id" value="{{$p->id}}">
                       ...
                       @if (($p->saved) != 0)
								<div class="pi-text">
									<b><a href='{{ route('single-product', $p->slug) }}' style="color:black;">{{$p->name}}</a></b>
									<b><a href='{{ route('single-product', $p->slug) }}' style="color:#0066ff;">{{$p->price}} EGP</a><br></b>
									<a href='{{ route('single-product', $p->slug) }}' style="color:gray;text-decoration: line-through;font-size:12px;">{{$p->dprice}} EGP</a><br>
									<a href='{{ route('single-product', $p->slug) }}' style="color:green; font-size:12px">ستوفر  {{$p->saved}} جنيه مصري </a>
								</div>
						@else
								<div class="pi-text">
										<b><a href='{{ route('single-product', $p->slug) }}' style="color:black;">{{$p->name}}</a></b>
										<a href='{{ route('single-product', $p->slug) }}' style="color:#0066ff;">{{$p->price}} EGP</a>
								</div>
						@endif
                    </div>
                </div>
            ...
            @if($products->count() > 8)
                <div class="text-center pt-5">
                    <a href="{{ route('frontendCategories') }}" class="site-btn sb-line sb-dark">المزيد</a>
                </div>
            @endif
        </div>
    </section>
